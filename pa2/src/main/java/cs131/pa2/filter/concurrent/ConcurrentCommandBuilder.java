	...
	private static boolean linkFilters(List<ConcurrentFilter> filters, String command){
		...
		while(iter.hasNext()) {
			...
			if(curr instanceof CdFilter || curr instanceof CatFilter || curr instanceof LsFilter || curr instanceof PwdFilter) {
				...
				// we need to close any file input streams opened by cat whenever it is not executed
				// due to error while linking filters
				for (ConcurrentFilter f : filters) {
					if (f instanceof CatFilter) {
						((CatFilter)f).terminate();
					}
				}
				return false;
			}
